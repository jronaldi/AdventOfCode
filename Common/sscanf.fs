...
let parseDecimal (x:string):Decimal = Decimal.Parse(x, System.Globalization.CultureInfo.InvariantCulture)
let parseBoolean (x:string):Boolean = Boolean.Parse(x)
...
let public sscanf (pf:PrintfFormat<_,_,_,_,'t>) s : 't =
